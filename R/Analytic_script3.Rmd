...
The naïve effect size (before correcting for selective reporting).
The adjusted 'true' effect size (after selective reporting correction).
We then calculated the percentage of cases in which the statistical significance of the effect changed (p < 0.05) after applying the selective reporting adjustment. This provides insight into how often selective reporting influences the strength of statistical conclusions.
...
The expected replication rate (ERR)—the probability that a statistically significant result would be replicated in a new study with the same design.
The false discovery rate (FDR)—the estimated proportion of significant findings that are likely to be false positives.
...
# compute absolute values of adjusted mu
d <- abs(df_eco$mu_adj)
# note that the `pwr` package cannot calculate sample sizes for small effects (`Cohen's d` below 0.001). So, we have to remove all effect sizes that are lower than 0.001.

d2 <- d[d > 0.001]

# compute n1 and n2
n1 <- sapply(d2, ttest.N)
n2 = sapply(d2, pwr.N)
...
The Figure 1 we reported in the main text can be reproduced with:
...
p5 <- dfp %>% filter(Field == "Economics") %>% 
  ...
  geom_text(aes(label = Design), hjust = 0.3, vjust = -2, size = 4) +
  geom_text(aes(label = est_2), hjust = 0.3, vjust = 3, size = 4) + 
  scale_fill_manual(values = paletteer_d("ggthemes::excel_Berlin")[4]) +
  scale_color_manual(values = paletteer_d("ggthemes::excel_Berlin")[4]) +
...
p6 <- dfp %>% filter(Field == "Environment") %>% 
  ...
  geom_text(aes(label = est_2), hjust = 0.3, vjust = 3, size = 4) +
  scale_fill_manual(values = paletteer_d("ggthemes::excel_Berlin")[2]) +
  scale_color_manual(values = paletteer_d("ggthemes::excel_Berlin")[2]) +
...
p7 <- dfp %>% filter(Field == "Medicine") %>% 
  ...
  geom_text(aes(label = est_2), hjust = 0.3, vjust = 3, size = 4) +
  scale_fill_manual(values = paletteer_d("ggthemes::excel_Berlin")[1]) +
  scale_color_manual(values = paletteer_d("ggthemes::excel_Berlin")[1]) +
...
p8 <- dfp %>% filter(Field == "Psychology") %>% 
  ...
  geom_text(aes(label = est_2), hjust = 0.3, vjust = 3, size = 4) +
  scale_fill_manual(values = paletteer_d("ggthemes::excel_Berlin")[3]) +
  scale_color_manual(values = paletteer_d("ggthemes::excel_Berlin")[3]) +
...
#png(filename = "Figure/Figure 1.png", width = 10, height = 10, units = "in", type = "windows", res = 800)
p <- (p7 + p6) / (p8 + p5) + 
  plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 16))
p
